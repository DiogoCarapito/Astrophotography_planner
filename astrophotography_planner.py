...
def get_observer(location_name):
    ...
    observer = Observer(
        latitude=latitude, longitude=longitude, name=location_name, timezone=timezone
    )
...
def get_targets(config):
    ...
    for name in config["targets"]:
        ...
        main_id = (
            result["main_id"][0].decode("utf-8")
            if isinstance(result["main_id"][0], bytes)
            else result["main_id"][0]
        )
        main_id = (
            re.sub(r"\s+", " ", main_id).strip().replace("NAME ", "")
...
def get_pretty_name_from_ids(ids_string, main_id):
    ...
    name_aliases = [
        ...
        and main_id.replace(" ", "") not in alias.replace(" ", "")
    ]
    if name_aliases:
...
def make_calendar_images(results, year):
    ...
    for month in range(1, 13):
        ...
        ax.set_axis_off()
        ax.set_title(
            calendar.month_name[month] + f" {year}", fontsize=16, fontweight="bold"
        )
...
def main(config):
    # Load configuration
    ## removed from the main function to allow for external loading
    # config = get_config()
    ...
    constraints = [
        AltitudeConstraint(
            min=config["min_altitude_deg"] * u.deg,
            max=config["max_altitude_deg"] * u.deg,
        ),
    ...
    for date in dates:
        ...
        try:
            astropy_datetime = Time(
                datetime.datetime.combine(date, datetime.time(12, 0))
            )

            observation_window_start = observer.sun_set_time(
                astropy_datetime, which="next", horizon=-12 * u.deg
            )
            observation_window_end = observer.sun_rise_time(
                astropy_datetime, which="next", horizon=-12 * u.deg
            )

            observable_minutes = (observation_window_end - observation_window_start).to(
                u.minute
            )
            ...
            if observation_window_start.mask or observation_window_end.mask:
                print(
                    "No astronomical twilight for this date. Night not valid for observations"
                )
                date_is_observable = False
            elif observable_minutes < min_minutes:
                print(
                    f"{observable_minutes:.0f} obsvervable minute for this night, but {min_minutes:.0f} is required. Night not valid for observations"
                )
                date_is_observable = False

            if date_is_observable:
                observation_window_start_local = observation_window_start.to_datetime(
                    timezone=observer.timezone
                ).strftime("%Y-%m-%d %H:%M")
                observation_window_end_local = observation_window_end.to_datetime(
                    timezone=observer.timezone
                ).strftime("%Y-%m-%d %H:%M")

                print(
                    f"Observing window: {observation_window_start_local} to {observation_window_end_local}"
                )
                ...
                for target in targets:
                    ...
                    constraint_masks = [
                        ...
                        )  # One array of booleans per constraint
                    ...
                    if target_observable_minutes >= min_minutes:
                        ...
                        try:
                            first_index = next(
                                i for i, ok in enumerate(combined_mask) if ok
                            )
                            observation_start_time = time_grid[first_index].to_datetime(
                                timezone=observer.timezone
                            )
                            observation_start_str = observation_start_time.strftime(
                                "%Y-%m-%d %H:%M"
                            )
...
if __name__ == "__main__":
    ## Load configuration ouside of the main function
    config = get_config()

    ## Execute the main logic with the loaded configuration
    main(config)
